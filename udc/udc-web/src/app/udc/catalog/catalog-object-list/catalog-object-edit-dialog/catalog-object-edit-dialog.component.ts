...
export class CatalogObjectEditComponent implements OnInit {
  heading = 'Edit Object';
  editForm: FormGroup;
  maxCharsForName = 100;
  ...
  constructor(public dialogRef: MatDialogRef<CatalogObjectEditComponent>, private fb: FormBuilder, private catalogService: CatalogService) {
  }
  ...
  onSubmit() {
    ...
    if (submitValue.modifiedObjectName.length > 0) {
      object.objectName = submitValue.modifiedObjectName;
    }
    if (submitValue.modifiedContainerName.length > 0) {
      object.containerName = submitValue.modifiedContainerName;
    }
    this.catalogService.getUserByName(submitValue.updatedUser)
      .subscribe(data => {
        object.updatedUser = submitValue.updatedUser;
