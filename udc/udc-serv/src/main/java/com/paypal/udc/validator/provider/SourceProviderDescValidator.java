...
@Component
public class SourceProviderDescValidator implements SourceProviderValidator {
    ...
    public void validate(final SourceProvider provider, final SourceProvider updatedProvider) throws ValidationError {
        if (provider.getSourceProviderDescription() != null && provider.getSourceProviderDescription().length() >= 0) {
            updatedProvider.setSourceProviderDescription(provider.getSourceProviderDescription());
        }
    }
}
