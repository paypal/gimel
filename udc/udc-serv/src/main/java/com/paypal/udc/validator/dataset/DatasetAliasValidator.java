...
@Component
public class DatasetAliasValidator implements DatasetValidator {
    ...
    public void validate(final Dataset dataSet, final Dataset updatedDataSet) throws ValidationError {
        if (dataSet.getStorageDataSetAliasName() != null && dataSet.getStorageDataSetAliasName().length() > 0) {
            updatedDataSet.setStorageDataSetAliasName(dataSet.getStorageDataSetAliasName());
        }
    }
}
