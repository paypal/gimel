...
case class TopicAndProps(topics: String, props: Map[String, String])
...
object ImplicitKafkaConverters {
  ...
  /**
   * @param topicAndProps A Tuple of (Comma-Separated topics, Properties)
   */
  implicit class TopicPropsConverter(topicAndProps: TopicAndProps) {
    ...
    /**
     * Converts a given Tuple of (Comma-Separated topics, Properties) into KafkaTopicAndPartitions
     *
     * @example val testing: Array[TopicAndPartition] = ("test_topic", Map("bootstrap.servers -> "localhost:9092")).toTopicAndPartitions
     * @return Array[TopicAndPartition]
     */
    def toTopicAndPartitions: Map[TopicPartition, (String, Int)] = {
      def MethodName: String = new Exception().getStackTrace.apply(1).getMethodName
      ...
      val client = AdminClient.create(KafkaUtilities.getConsumerPropertiesPerBroker(topicAndProps.props))
      import scala.collection.JavaConverters._
      try {
        client.describeTopics(
          topicAndProps.topics.split(",").toList.asJava
        ).all().get().asScala.flatMap { topicMetadata => {
          topicMetadata._2.partitions().asScala.map {
            partitionMetadata =>
              partitionMetadata.isr()
    ...
    /**
     * Converts a given Tuple of (Comma-Separated topics, Properties) into Array[OffsetRange] available currently in Kafka Cluster
     *
     * @example val kafkaOffsets:Array[OffsetRange] = ("test_topic", Map("bootstrap.servers -> "localhost:9092")).toKafkaOffsetsPerPartition
     * @return Array[OffsetRange]
     *
     */
    def toKafkaOffsetsPerPartition: Array[OffsetRange] = {
      ...
      val topicAndPartitions: Map[TopicPartition, (String, Int)] = topicAndProps.toTopicAndPartitions
      import scala.collection.JavaConverters._
      val partitions = topicAndPartitions.keySet.asJava
      ...
      val kafkaConsumer = KafkaUtilities.getKafkaConsumer(Some(
        KafkaUtilities.getConsumerPropertiesPerBroker(topicAndProps.props)
      ))
      try {
        val beginningOffsets: util.Map[TopicPartition, lang.Long] = kafkaConsumer.beginningOffsets(partitions)
        val endOffsets: util.Map[TopicPartition, lang.Long] = kafkaConsumer.endOffsets(partitions)
