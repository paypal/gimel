...
class KafkaClientConfiguration(val props: Map[String, Any]) {
  ...
  val kafkaConsumerProps: Map[String, String] = scala.collection.immutable.Map(
  ...
  val kafkaOptionsLoader: Option[String] = tableProps.get(KafkaConfigs.kafkaOptionsLoaderKey)

  logger.info(s"Fields Initiated --> ${this.getClass.getFields.map(f =>
    s"${f.getName} --> ${f.get().toString}").mkString("\n")}")
