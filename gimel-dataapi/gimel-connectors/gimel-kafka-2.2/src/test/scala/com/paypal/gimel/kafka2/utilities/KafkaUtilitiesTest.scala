...
class KafkaUtilitiesTest extends FunSpec with Matchers with BeforeAndAfterAll {
  val kafkaCluster = new EmbeddedKafkaCluster(2)
  ...
  protected override def afterAll(): Unit = {
    kafkaCluster.deleteTopic(topic + "_10")
  }
  ...
  describe("inStreamCheckPoint") {
    it("should checkpoint a given OffsetRange in zookeeper") {
      ...
      KafkaUtilities.inStreamCheckPoint(kafkaCluster.zKConnectString(), Seq(zkNode + topic), offsetRanges)
      val lastCheckpoint = KafkaUtilities.getLastCheckPointFromZK(kafkaCluster.zKConnectString(), Seq(zkNode + topic))
  ...
  describe("getNewOffsetRangeForReader") {
    it("should get the lastCheckpoint if exists from zookeeper") {
      ...
      KafkaUtilities.inStreamCheckPoint(kafkaCluster.zKConnectString(), Seq(zkNode + topic + "_2"), offsetRanges)
    ...
    it("should get the available offset range and limit it using fetchRowsOnFirstRun if last checkpoint does not exist") {
      ...
      KafkaUtilities.inStreamCheckPoint(kafkaCluster.zKConnectString(), Seq(zkNode + topic + "_3"), offsetRanges)
  ...
  describe("clearCheckPoint") {
    it("should clear the lastCheckpoint if exists") {
      ...
      KafkaUtilities.inStreamCheckPoint(kafkaCluster.zKConnectString(), Seq(zkNode + topic + "_4"), offsetRanges)

      // clear the last checkpoint
      KafkaUtilities.clearCheckPoint(kafkaCluster.zKConnectString(), Seq(zkNode + topic + "_4"), "")
      val lastCheckpoint = KafkaUtilities.getLastCheckPointFromZK(kafkaCluster.zKConnectString(), Seq(zkNode + topic + "_4"))
  ...
  describe("getLastCheckPointFromZK") {
    it("should get the lastCheckpoint if exists") {
      ...
      KafkaUtilities.inStreamCheckPoint(kafkaCluster.zKConnectString(), Seq(zkNode + topic + "_5"), offsetRanges)
      val lastCheckpoint = KafkaUtilities.getLastCheckPointFromZK(kafkaCluster.zKConnectString(), Seq(zkNode + topic + "_5"))
    ...
    it("should return None if lastCheckpoint does not exist") {
      val lastCheckpoint = KafkaUtilities.getLastCheckPointFromZK(kafkaCluster.zKConnectString(), Seq(zkNode + topic + "_6"))
      assert(lastCheckpoint == None)
    }
  }
