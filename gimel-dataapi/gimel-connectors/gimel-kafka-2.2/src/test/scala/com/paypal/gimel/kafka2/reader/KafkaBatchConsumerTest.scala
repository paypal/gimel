...
import com.paypal.gimel.kafka2.utilities.KafkaOptionsLoaderUtils
...
class KafkaBatchConsumerTest extends FunSpec with SharedSparkSession {
  ...
  protected override def afterAll(): Unit = {
    ...
    kafkaCluster.deleteTopic(topic + "_1")
  }
  ...
  describe("getOffsetRange") {
    it ("should connect to zookeeper to get the last checkpoint if found otherwise gets the available offsets for each kafka partition") {
      ...
      val kafkaTopicsOptionsMap : Map[String, Map[String, String]] =
        KafkaOptionsLoaderUtils.getAllKafkaTopicsOptions(new KafkaClientConfiguration(datasetProps))
      val (resultOffsetRanges, parallelizedOffsetRanges) = KafkaBatchConsumer.getOffsetRange(new KafkaClientConfiguration(datasetProps), kafkaTopicsOptionsMap)
