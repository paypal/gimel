...
import com.paypal.gimel.kafka2.utilities.{BrokersAndTopic, KafkaOptionsLoaderUtils, KafkaUtilitiesException, TopicAndProps}
...
object KafkaStreamConsumer {
   ...
   *
   * @param conf : KafkaClientConfiguration object that holds the configuration paremeters
   * @param kafkaOptionsMap : Map of topics to options
   * @return Starting Offsets in a Map[TopicPartition, Long]
   */
  private def getStartOffsets(conf: KafkaClientConfiguration, kafkaOptionsMap: Map[String, Map[String, String]]) = {
    if (conf.kafkaCustomOffsetRange.isEmpty()) {
      val lastCheckPoint: Option[Array[OffsetRange]] = getLastCheckPointFromZK(conf.zkHostAndPort, conf.zkCheckPoints)
      val availableOffsetRange: Option[Array[OffsetRange]] = kafkaOptionsMap.map(x =>
        TopicAndProps(x._1, x._2).toKafkaOffsetsPerPartition).reduceOption(_.union(_))
      val availableOffsetRanges: Array[OffsetRange] = availableOffsetRange match {
        case None =>
  ...
  def createStructuredStream(sparkSession: SparkSession, conf: KafkaClientConfiguration, props: Map[String, Any]): StructuredStreamingResult = {
    try {
      ...
      val startOffsetsForStream: Map[TopicPartition, Long] = getStartOffsets(conf, kafkaTopicsOptionsMap)
      ...
      val dataStreamReader: DataStreamReader = sparkSession
        ...
        .options(eachKafkaTopicToOptionsMap.getOrElse(topics, Map.empty[String, String]))
