...
/*
 * Deserializer class for String data
 */
class StringDeserializer extends Deserializer {
  override def deserialize(dataframe: DataFrame, props: Map[String, Any] = Map.empty): DataFrame = {
    ...
    if (!dataframe.columns.contains(conf.columnToDeserialize)) {
      ...
      val deserializedDF = dataframe.withColumn(kafkaValueMessageColAlias, dataframe(conf.columnToDeserialize).cast(StringType))
