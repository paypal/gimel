...
class JsonDynamicDeserializerTest extends FunSpec with Matchers with SharedSparkSession {
  val jsonDeserializer = new JsonDynamicDeserializer

  describe("deserialize") {
    it ("should return deserialized dataframe") {
      val dataframeJson = mockDataInDataFrame(10)
      val dataFrame = mockDataInDataFrameWithJsonStringBytes(10)
      val deserializedDF = jsonDeserializer.deserialize(dataFrame)
      assert(dataframeJson.except(deserializedDF).count() == 0)
    }
  }
  ...
  describe("deserialize with columnToDeserialize not present in input dataframe") {
    it("it should throw error if " + GenericDeserializerConfigs.columnToDeserializeKey + " is not present in input dataframe") {
      ...
      val exception = intercept[IllegalArgumentException] {
        jsonDeserializer.deserialize(dataFrame, props)
      }
